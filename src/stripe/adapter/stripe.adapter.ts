...
import { PLAN_CODES } from "src/common/constants/plan.constants";
...
export class StripeAdapter implements IPaymentAdapter {
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      // Ensure successUrl has ?session_id={CHECKOUT_SESSION_ID} if not present
      let finalSuccessUrl = successUrl || this.configService.get<string>("STRIPE_SUCCESS_URL", "http://localhost:3000/success");
      if (!finalSuccessUrl.includes("{CHECKOUT_SESSION_ID}")) {
        finalSuccessUrl += finalSuccessUrl.includes("?") ? "&session_id={CHECKOUT_SESSION_ID}" : "?session_id={CHECKOUT_SESSION_ID}";
      }
      ...
      const sessionData: Stripe.Checkout.SessionCreateParams = {
        ...
        cancel_url: cancelUrl || this.configService.get<string>("STRIPE_CANCEL_URL", "http://localhost:3000/cancel"),
        metadata,
      };
